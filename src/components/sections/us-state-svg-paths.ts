/**
 * US state SVG path data for the 50 states + Washington DC.
 *
 * Path data adapted from the open-source `react-usa-map` project by Gabriela
 * D'Avila Ferrara (https://github.com/gabidavila/react-usa-map), which is
 * released under the MIT License. The original blank US states map paths are
 * derived from public-domain Wikimedia/Albers USA projection sources.
 *
 * The intended SVG viewBox is "0 0 959 593" (Albers USA projection that
 * already places Alaska and Hawaii as insets inside the main viewport).
 *
 * `labelX` / `labelY` are the bounding-box midpoints of each state path,
 * suitable for placing a 2-letter abbreviation label at the visual center.
 * For very small jurisdictions (DE, NJ, CT, RI, MA, DC) the parent
 * component should size labels accordingly to avoid overlap.
 */
...
export const REGION_PATHS: Record<string, PathData> = {
  "alabama": {
  ...
  "district-of-columbia": {
    d: "M805.92,253.71 L803.21,256.42 L799.39,256.42 L796.68,253.71 L796.68,249.89 L799.39,247.18 L803.21,247.18 L805.92,249.89 Z",
    labelX: 801.3,
    labelY: 251.8,
  },
  "florida": {
