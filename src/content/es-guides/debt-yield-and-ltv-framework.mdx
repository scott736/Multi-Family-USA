...
description: "Marco práctico de debt yield y LTV para equilibrar apalancamiento, cobertura y velocidad de ejecución en multifamiliar."
...
- q: ¿Cómo interactúan debt yield y LTV en el dimensionamiento?
  a: Los prestamistas suelen aplicar ambos y dimensionar por la restricción más
    ajustada, especialmente cuando la durabilidad del NOI es incierta.
- q: ¿Qué métrica suele limitar primero en activos estabilizados?
  a: En muchas ejecuciones estabilizadas, el apalancamiento o debt yield puede
    limitar antes que el DSCR según cupón, calidad de NOI y programa.
- q: ¿Cómo usar debt yield en negociación de term sheets?
  a: Presente debt yield in-place y estabilizado con respaldo para que el comité
    valide desembolsos sin preguntas repetidas sobre NOI.
...
> **Nota editorial:** Metadatos en español para esta guía. El cuerpo se está localizando; la [versión en inglés](/learn/debt-yield-and-ltv-framework/) sigue siendo la referencia editorial más actualizada.


import KeyTakeaways from "@/components/dscr/KeyTakeaways.astro";

## How do debt yield and LTV work together on multifamily loans?

Debt yield and LTV are parallel guardrails: debt yield ties loan balance to NOI while LTV ties balance to value. Lenders often cap proceeds at whichever metric is tighter after stress, so sponsors should model both before requesting quotes on 5+ unit US multifamily deals.

## Introduction

Debt Yield and LTV: A Practical Framework is about balancing leverage targets against lender risk controls and long-term refinancing flexibility. In US commercial multifamily financing, lenders underwrite 5+ unit properties using a combination of historical performance, forward-looking business plan credibility, sponsorship depth, and market liquidity. If any one of those elements is weak, proceeds and terms adjust quickly.
...
<KeyTakeaways
  points={[
    "Debt yield is a lender risk floor, while LTV depends on valuation assumptions.",
    "High leverage with weak debt yield increases refinance and extension risk.",
    "Use blended constraints from debt yield, DSCR, and LTV to define true proceeds.",
    "Cap rate expansion scenarios should be built into every hold-period refinance model.",
    "Permanent execution improves when leverage discipline is set before bids are requested.",
  ]}
/>
...
Commercial multifamily lenders do not size debt from a single metric. They typically evaluate [DSCR](/tools/commercial-dscr-calculator/), [debt yield](/tools/debt-yield-calculator/), and leverage simultaneously, then choose the most constraining outcome. Build your model the same way so you know where your true proceeds ceiling sits.
